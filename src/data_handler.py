...
def create_dataset(
    ...
    -----
        ...
        K (int): The number of channel cross-correlation lags (relevant only for SubspaceNetCC model).
        G (int): The number of non-overlapping segments (relevant only for SubspaceNetCC model).
        L (int): The segment length (relevant only for SubspaceNetCC model).
        ref_channel (int): The reference channel index (relevant only for SubspaceNetCC model).
    ...
    if model_type.startswith("SubspaceNetCC") and None in (K, G, L, ref_channel):
        raise ValueError(
            "create_dataset: K, G, L, and ref_channel must be provided for SubspaceNetCC model"
        )
    ...
    if model_type.startswith("DeepCNN") and phase.startswith("train"):
        ...
        for i in tqdm(range(samples_size)):
            ...
            if model_type.startswith("SubspaceNetCC"):
                # 新分支：通道互相关张量
                X_model = create_channel_cross_correlation_tensor(
                    X, K=K, G=G, L=L, ref_channel=ref_channel
                ).to(torch.float)
    ...
    if save_datasets:
        ...
        model_dataset_filename = f"{model_type}_DataSet" + set_dataset_filename(
            system_model_params, samples_size, **model_filename_kwargs
        )
...
def load_datasets(
    ...
    if model_type.startswith("SubspaceNetCC") and None in (K, G, L, ref_channel):
        raise ValueError(
            "load_datasets: K, G, L, and ref_channel must be provided for SubspaceNetCC model"
        )
    ...
    model_dataset_filename = f"{model_type}_DataSet" + set_dataset_filename(
        system_model_params, test_samples_size, **model_filename_kwargs
    )
    generic_dataset_filename = f"Generic_DataSet" + set_dataset_filename(
...
def set_dataset_filename(
    ...
    -----
        system_model_params (SystemModelParams): an instance of SystemModelParams.
        samples_size (float): The size of the overall dataset.
        K (int): The number of channel cross-correlation lags.
        G (int): The number of non-overlapping segments.
        L (int): The segment length.
        ref_channel (int): The reference channel index.
    ...
    if K is not None or G is not None or L is not None or ref_channel != 0:
        suffix_filename = (
            suffix_filename[:-3]
            + f"K={K}_G={G}_L={L}_ref_channel={ref_channel}_"
            + ".h5"
        )
    return suffix_filename


def channel_cross_correlation_matrix(
    X_segment: torch.Tensor, K: int, ref_channel: int = 0
):
    """
    计算单段数据的参考通道互相关矩阵。

    参数:
        X_segment: 单段数据，形状 [N, L]，复数
        K: 保留的延时点数（tau = 0, 1, ..., K-1）
        ref_channel: 参考通道索引，默认 0

    返回:
        torch.Tensor，形状 [2K, N]，实数 float
        前 K 行是 K 个延时下互相关向量的实部，后 K 行是虚部

    互相关定义（按权利要求 5）:
        r_tau[n] = (1/(L-tau)) * sum_{t=0}^{L-tau-1}
                   conj(x_ref(t)) * x_n(t+tau)
    """
    if X_segment.ndim != 2:
        raise ValueError("X_segment must have shape [N, L]")

    N, L = X_segment.shape
    if K <= 0:
        raise ValueError("K must be positive")
    if K > L:
        raise ValueError("K must be less than or equal to the segment length L")
    if ref_channel < 0 or ref_channel >= N:
        raise ValueError("ref_channel is out of range")

    X_segment = X_segment.to(dtype=torch.complex64)
    R = torch.zeros((K, N), dtype=torch.complex64, device=X_segment.device)

    for tau in range(K):
        ref = torch.conj(X_segment[ref_channel, : L - tau])
        delayed = X_segment[:, tau:L]
        R[tau] = torch.sum(ref.unsqueeze(0) * delayed, dim=1) / (L - tau)

    return torch.cat((torch.real(R), torch.imag(R)), dim=0).to(torch.float)


def create_channel_cross_correlation_tensor(
    X: torch.Tensor, K: int, G: int, L: int, ref_channel: int = 0
):
    """
    将整段观测 X 划分为 G 个非重叠段，对每段调用 channel_cross_correlation_matrix。

    参数:
        X: 完整观测，形状 [N, T]，复数
        K, G, L, ref_channel: 同上

    返回:
        torch.Tensor，形状 [G, 2K, N]，实数 float

    要点:
        - 必须 L*G <= T，否则 raise ValueError
        - 第 g 段 = X[:, g*L:(g+1)*L]
        - G 段结果沿第 0 维 stack
    """
    if X.ndim != 2:
    ...
    T = X.shape[1]
    if L * G > T:
        raise ValueError("L*G must be less than or equal to T")

    segments = []
    for g in range(G):
        X_segment = X[:, g * L : (g + 1) * L]
        segments.append(channel_cross_correlation_matrix(X_segment, K, ref_channel))
