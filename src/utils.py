...
def sum_of_diags_torch(matrix: torch.Tensor):
    ...
    diag_sum = []
    n = matrix.shape[-1]
    diag_index = torch.arange(-n + 1, n, device=matrix.device)
    for idx in diag_index:
        diag = torch.diagonal(matrix, offset=int(idx.item()), dim1=-2, dim2=-1)
        diag_sum.append(torch.sum(diag, dim=-1))
    return torch.stack(diag_sum, dim=-1)
...
def find_roots_torch(coefficients: torch.Tensor):
    ...
    if coefficients.dim() == 1:
        ...
        A = torch.zeros(
            n, n, dtype=coefficients.dtype, device=coefficients.device
        )
        A[1:, :-1] = torch.eye(n - 1, dtype=coefficients.dtype, device=coefficients.device)
        denom = coefficients[0]
        if torch.abs(denom) == 0:
            denom = denom + 1e-9
        A[0, :] = -coefficients[1:] / denom
        roots = torch.linalg.eigvals(A)
        return roots

    batch_size, coefficients_len = coefficients.shape
    n = coefficients_len - 1
    A = torch.zeros(
        batch_size, n, n, dtype=coefficients.dtype, device=coefficients.device
    )
    A[:, 1:, :-1] = torch.eye(
        n - 1, dtype=coefficients.dtype, device=coefficients.device
    ).unsqueeze(0)
    denom = coefficients[:, :1]
    denom = torch.where(torch.abs(denom) == 0, denom + 1e-9, denom)
    A[:, 0, :] = -coefficients[:, 1:] / denom
    roots = torch.linalg.eigvals(A)
    return roots
...
def gram_diagonal_overload(Kx: torch.Tensor, eps: float, batch_size: int = None):
    ...
    Kx = Kx.to(device)
    # Batched Gram operation: [B, N, N]^H @ [B, N, N].
    Kx_garm = torch.matmul(torch.conj(Kx).transpose(-2, -1), Kx)
    eps_addition = eps * torch.eye(
        Kx_garm.shape[-1], dtype=Kx_garm.dtype, device=Kx_garm.device
    )
    return Kx_garm + eps_addition
