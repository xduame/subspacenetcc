...
def _to_numpy(value):
  """Convert tensors to CPU numpy arrays before handing values to Matplotlib."""
  if value is None:
    return None
  if isinstance(value, torch.Tensor):
    return value.detach().cpu().numpy()
  if isinstance(value, list):
    return np.asarray([_to_numpy(item) for item in value])
...
def plot_spectrum(predictions: np.ndarray, true_DOA: np.ndarray, system_model=None,
  ...
  """
  predictions = None if predictions is None else np.squeeze(_to_numpy(predictions))
  true_DOA = _to_numpy(true_DOA)
  spectrum = _to_numpy(spectrum)
  roots = None if roots is None else np.squeeze(_to_numpy(roots))
...
def plot_root_music_spectrum(roots: np.ndarray, predictions: np.ndarray,
    ...
    output_path = Path("data") / "spectrums" / "{}_spectrum.pdf".format(algorithm)
