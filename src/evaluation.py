...
def evaluate_dnn_model(
    ...
    if plot_spec and model_type.startswith("SubspaceNet"):
        ...
        if DOA_all is not None and roots is not None:
            plot_spectrum(
            ...
            if not algorithm.startswith("esprit"):
                if plot_spec and i == len(dataset.dataset) - 1:
                    if "r-music" in algorithm:
                        _, roots, predictions_all, _, _ = method_output
                        plot_spectrum(
...
def evaluate_model_based(
    ...
    Args:
        ...
        algorithm (str): The algorithm to use (e.g., "music", "mvdr", "esprit",
            "r-music", "cc-toeplitz-r-music"). Defaults to "music".
        figures (dict): Dictionary containing figure objects for plotting. Defaults to None.
        cc_L (int): Segment length for CC-Toeplitz methods.
        cc_G (int): Number of segments for CC-Toeplitz methods.
        cc_ref_channel (int): Reference channel for CC-Toeplitz methods.
    ...
    for i, data in enumerate(dataset):
        ...
        if "r-music" in algorithm and "cc-toeplitz" not in algorithm:
        ...
        # MUSIC algorithms
        elif "music" in algorithm and "cc-toeplitz" not in algorithm:
            music = MUSIC(system_model)
        ...
        # CC-Toeplitz Root-MUSIC algorithm
        elif "cc-toeplitz" in algorithm:
            from src.methods import CrossCorrToeplitzRootMUSIC

            if cc_L is None or cc_G is None:
                raise ValueError(
                    "evaluate_model_based: cc_L and cc_G must be provided for cc-toeplitz algorithms."
                )
            cc_method = CrossCorrToeplitzRootMUSIC(
                system_model, L=cc_L, G=cc_G, ref_channel=cc_ref_channel
            )
            predictions, roots, predictions_all, _, M = cc_method.estimate(X)
            predictions = add_random_predictions(M, predictions, algorithm)
            loss = criterion(predictions, doa * R2D)
            loss_list.append(loss)
